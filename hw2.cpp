...
int solve(vector<vector<double>> bars){
  ...
  //  это случается, если мы прошли барьеры до крайнего и не врезались в него
  if (!right){ // движение было влево
    return int(0); // ответ - нулевая зона
  }else{ // движение было вправо
...
int main(int argc, char** argv) {
  ...
  double x = 0;
  double h = 0;
  double n = 1; // будем добавлять к информации о барьере его номер, который не будет нигде меняться
